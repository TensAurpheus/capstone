...
def split_scale(df, target_cols='y', test_size=0.2, val_size=0.1, scale=True):
    ...
    exclude_cols = ['open', 'high', 'low', 'close', 'atr_14', 'atr_pct']
    ...
    print(f"[INFO] Total features: {len(feature_cols)} | Scaled: {len(scale_cols)} | Excluded: {exclude_cols}")
...
class CryptoDataset(Dataset):
    """PyTorch Dataset for sequential crypto data windows."""
...
def triple_barrier_label(df, close='close', high='high', low='low',
    ...
    returns: labels 
    """
    closes, highs, lows, volatility = (
        df[close].values, df[high].values, df[low].values, df[volatility].values
    )
